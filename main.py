...
app.config['SQLALCHEMY_DATABASE_URI'] ='sqlite:///akash.db'
...
class Applys(db.Model):
    ...
    def calculate_late_return_charge(self):
        # Calculate the due datetime by adding 2 minutes to the due_date
        due_datetime = datetime.combine(self.due_date, datetime.min.time()) + timedelta(minutes=2)

        if datetime.now() > due_datetime:
            # Calculate the number of minutes late
            minutes_late = (datetime.now() - due_datetime).seconds // 60
            # Charge 2 rs per minute
            return 2 * minutes_late
        else:
            return 0
...
def student_dashboard():
    if 'prn' in session:
        ...
        if request.method == 'POST':
            ...
            books = Books.query.filter((Books.name.ilike(f"%{search_query}%")) | (Books.writer.ilike(f"%{search_query}%"))).all()
        else:
            # Query all books if the form is not submitted
            books = Books.query.all()

        return render_template('dashboard.html', student_name=student_name, student_prn=student_prn, books=books)
    else:
        flash('You must be logged in to access the dashboard.', 'error')
        return redirect('/login')
...
def addbook():
    if 'admin_logged_in' in session:
        if (request.method == 'POST'):
            ...
            entry = Books(b_id=book_id, name=name, writer=writer,  copy=copy)
...
def mark_returned(app_id):
    if 'admin_logged_in' in session:
        ...
        if application:
            ...
            # Calculate and update the late return charge
